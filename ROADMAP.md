...
La premessa relativa a un singolo `script.py` non corrisponde più allo stato del repository: nel branch `main` il file non è presente. **OSI-CYBER-EXPLORER è già una Single Page Application educativa, bilingue e interamente client-side**, costruita con React 19, TypeScript, Vite 8, Tailwind CSS 4, Zustand, Motion e Vitest.
...
- [x] **SEC-02 — Applicare limiti pre-parse agli input JSON.** In `AutomationLab`/`src/lib/automation.ts` definire soglie condivise per byte, profondità e numero di nodi prima e durante la visita. `flattenJsonDocument` limita nodi e profondità solo dopo `JSON.parse`, mentre `findSensitiveJsonPaths` percorre oggi l'intero oggetto senza gli stessi limiti. **Completato quando:** input oltre soglia falliscono in modo controllato e test boundary/fuzz coprono byte, depth, nodes e stack exhaustion; valutare `fast-check`. **Esito (24/09/2026):** introdotti limiti condivisi e immutabili di 64 KiB UTF-8, profondità strutturale 12 e 200 nodi. Un pre-scan consapevole di stringhe ed escape verifica byte e profondità prima di `JSON.parse`; parsing, flattening e ricerca delle chiavi sensibili usano ora un'unica ispezione iterativa, senza ricorsione e con conteggio dei nodi durante la visita. Gli errori espongono codici controllati (`JSON_INVALID`, `JSON_TOO_MANY_BYTES`, `JSON_TOO_DEEP`, `JSON_TOO_MANY_NODES`) e `AutomationLab` analizza ogni input una sola volta. Aggiunti test di confine, UTF-8 multibyte, parentesi nelle stringhe, parità dei limiti, payload con 10.000 livelli e corpus fuzz deterministico: 44 file/350 test superati. `fast-check` è stato valutato ma rinviato a NET-02, dove le proprietà di conversione IP ne giustificano l'introduzione e il riuso.

- [x] **SEC-03 — Limitare il server di sviluppo alla loopback.** Sostituire il default `vite --host=0.0.0.0` con `127.0.0.1`; offrire un comando `dev:network` esplicito e documentato solo per chi necessita accesso LAN. **Completato quando:** `npm run dev` non espone il progetto sulla rete locale per impostazione predefinita. **Esito (25/09/2026):** `npm run dev` usa ora esplicitamente `--host=127.0.0.1`; l'esposizione su tutte le interfacce è disponibile solo tramite il comando intenzionale `npm run dev:network`, configurato con `--host=0.0.0.0`. README inglese, README italiano e descrizione spiegano la differenza, l'uso esclusivo su LAN fidate e il divieto di impiegare il server Vite come endpoint pubblico o di produzione. Il comando predefinito è stato avviato con `--strictPort` e Vite ha esposto esclusivamente `http://127.0.0.1:3000/`.

- [x] **SEC-04 — Ridurre i privilegi della CI.** Dichiarare `permissions: contents: read`, usare `persist-credentials: false` in `actions/checkout` (il token non deve restare nella configurazione git del runner), aggiungere `timeout-minutes` e fissare le GitHub Actions a commit SHA verificati, con aggiornamenti automatizzati controllati. **Completato quando:** il token del workflow è read-only e tutte le action sono immutable-pinned. **Esito (26/09/2026):** workflow limitato a `contents: read`, checkout con `persist-credentials: false`, timeout di 15 minuti e action fissate a SHA verificati. I pin CI sono stati aggiornati durante SEC-06 a `actions/checkout` `3d3c42e5aac5ba805825da76410c181273ba90b1` (`v7.0.1`) e `actions/setup-node` `820762786026740c76f36085b0efc47a31fe5020` (`v7.0.0`). Aggiunto Dependabot settimanale per GitHub Actions e npm, con massimo 5 PR aperte per ecosistema. Verifica della CI su entrambe le versioni Node e ispezione dei job completate dopo il push.

### P1 — Hardening applicativo e del deployment

- [x] **SEC-05 — Introdurre security header sul deploy Vercel.** Configurare `vercel.json` con CSP prima in `Report-Only` e poi enforced, almeno `default-src 'self'`, `object-src 'none'`, `base-uri 'none'`, `frame-ancestors 'none'`; aggiungere `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy` e HSTS sul solo endpoint HTTPS di produzione. Considerare l'immagine Unsplash attuale nella guida prima di restringere `img-src`. Come passo finale, dato che il codice non usa sink DOM testuali, valutare `require-trusted-types-for 'script'` (Trusted Types) in Report-Only per rendere strutturalmente impossibile una futura DOM XSS. **Completato quando:** gli header sono verificati sulla demo e non interrompono Motion, font, asset o lazy chunks. **Esito (26/09/2026):** aggiunto `vercel.json` con CSP distribuita prima in Report-Only e poi applicata dopo la verifica della demo. La policy limita script, connessioni, font, frame, form, worker e media; `img-src` ammette soltanto asset locali, `data:` e l'immagine esistente su `images.unsplash.com`, mentre `style-src` mantiene `'unsafe-inline'` perché Motion usa attributi di stile. Aggiunti `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `X-Frame-Options`, `Cross-Origin-Opener-Policy` e HSTS. `require-trusted-types-for 'script'` resta in Report-Only. Verificati sul deploy HTTPS gli header effettivi, l'assenza di violazioni CSP/Trusted Types, il caricamento dell'app, dei lazy chunk e dell'immagine della guida.

- [x] **SEC-06 — Automatizzare SAST, dependency e secret scanning.** Aggiungere CodeQL JavaScript/TypeScript, Dependabot per npm e GitHub Actions, Gitleaks o secret scanning equivalente, OSV/npm audit e SBOM CycloneDX. Bloccare inizialmente solo nuovi finding High/Critical, con eccezioni versionate. **Completato quando:** ogni PR produce risultati leggibili e una vulnerabilità non può essere ignorata senza motivazione. **Esito (26/09/2026):** aggiunti workflow separati per CodeQL JavaScript/TypeScript con query `security-extended`, Gitleaks sull'intera cronologia, gate `npm audit` High/Critical, OSV-Scanner con confronto dei soli nuovi finding nelle PR e scansione completa schedulata, e SBOM CycloneDX JSON generata con Syft e conservata come artifact per 14 giorni. Tutte le action dirette sono fissate a commit SHA verificati; Dependabot continua a proporne l'aggiornamento. Le soppressioni partono vuote e richiedono configurazione stretta, ledger versionato con responsabile, motivazione, controllo compensativo e scadenza massima di 90 giorni. Verificati su `main` CI (Node 22/24), CodeQL, Gitleaks, audit, OSV (291 pacchetti, nessun finding) e generazione SBOM: tutti i job sono verdi e i report SARIF/artifact sono disponibili nelle rispettive run.

- [x] **SEC-07 — Centralizzare la validazione degli input interattivi.** Applicare funzioni pure o schemi `zod`/`valibot` a JSON, path REST simulato, IP/subnet, wildcard, VLAN e valori numerici; introdurre limiti di lunghezza e rifiuto dei caratteri di controllo. Il campo `resource` deve accettare solo un path relativo e dichiarare chiaramente che nessuna richiesta viene inviata. **Completato quando:** errori e limiti sono coerenti, bilingui e coperti da test. **Esito (26/09/2026):** introdotto `src/lib/inputValidation.ts` come livello puro e condiviso, senza nuove dipendenze, con limiti immutabili, rifiuto dei caratteri di controllo, parsing stretto degli interi e messaggi IT/EN basati su codici stabili. Applicato ai documenti JSON, agli status HTTP, a IPv4/prefissi e VLSM, agli intervalli wildcard e alle VLAN singole/liste. Il path REST accetta esclusivamente path relativi all'host fino a 256 caratteri, rifiuta URL assoluti, authority, query, fragment, slash/backslash codificati e traversal anche percent-encoded; l'interfaccia dichiara esplicitamente che la simulazione non invia richieste. I campi numerici restano stringhe durante l'editing, quindi vuoto, decimali e valori fuori range non vengono più convertiti silenziosamente in `0`, `NaN` o estremi validi. Il pianificatore VLSM limita inoltre etichette e segmenti. Aggiunti 8 test dedicati per confini, normalizzazione, controllo caratteri, traversal, parità bilingue e immutabilità: suite completa verde su 45 file/358 test, con type-check, lint, build e audit senza vulnerabilità.

- [x] **SEC-08 — Validare e versionare `localStorage`.** Aggiungere versione, migrazione e schema di rehydration per `language`, `audioEnabled`, `simSpeed` e `hasSeenGuide`; ignorare dati corrotti o valori fuori dominio. Non cifrare queste preferenze: non sono segreti. **Completato quando:** testano storage manomesso, schema precedente e ripristino dei default. **Esito (26/09/2026):** introdotto uno schema puro e tipizzato che ricostruisce esclusivamente le quattro preferenze ammesse, ripristina il default del singolo campo mancante o fuori dominio e scarta ogni proprietà estranea. Lo storage Zustand è ora alla versione `1`: la migrazione conserva e sanitizza il precedente formato non versionato (`0`), mentre versioni sconosciute ripartono dai default senza tentare conversioni ambigue. La rehydration applica lo schema anche ai dati già marcati con la versione corrente, così uno storage manomesso non può ripristinare stato di sessione. Le velocità `0.5x`, `1x` e `2x` sono definite in un'unica allowlist condivisa tra store e UI. Le preferenze restano intenzionalmente in chiaro perché non contengono segreti. Aggiunti test puri e di integrazione `localStorage` per JSON corrotto, payload alterati, chiavi extra, campi misti validi/non validi, schema precedente e versioni future: suite completa verde su 47 file/379 test, con type-check, lint, build e audit senza vulnerabilità.

- [x] **SEC-09 — Rafforzare il rilevamento didattico dei segreti.** Estendere `SENSITIVE_KEY` a varianti come `authorization`, `access_token`, `client_secret` e `secretKey`, mantenendo sempre i valori redatti. Aggiungere test su maiuscole, separatori, Unicode, falsi positivi e limiti. **Completato quando:** il warning non espone mai il valore e resta esplicito che non sostituisce un secret scanner. **Esito (26/09/2026):** sostituita la regex generica con un classificatore lineare basato su token normalizzati, suffissi sensibili e allowlist di chiavi compatte. Il rilevamento copre `authorization`, password/passphrase, credenziali, token di accesso/refresh/ID/API, `client_secret`, `secretKey`, chiavi private e community SNMP in camelCase, maiuscole, separatori diversi, forme Unicode compatibili, accenti e separatori invisibili. Le corrispondenze dipendono dalla posizione semantica finale, evitando falsi positivi didattici come `tokenBucket`, `passwordPolicy`, `authorizationServer`, `privateKeyAlgorithm` e `apiKeyName`. I valori vengono sostituiti con `[REDACTED]` durante l'ispezione, prima della UI e prima di restituire i nodi; la redazione si propaga a tutti i discendenti di contenitori sensibili come `credentials`. Il warning mostra soltanto i path e continua a dichiarare in italiano e inglese che il controllo euristico non sostituisce un secret scanner. Limiti pre-parse, profondità e numero di nodi restano invariati. Aggiunti test logici e UI contro regressioni e leakage: suite completa verde su 48 file/385 test, con type-check, lint, build e audit senza vulnerabilità.
...
| 2026-09-24 | SEC-02: pre-scan limitato e visita JSON iterativa condivisa | Blocca byte e profondità prima del parsing, applica lo stesso budget di nodi a tutte le analisi ed elimina il rischio di stack exhaustion senza aggiungere una dipendenza solo per il fuzzing |
| 2026-09-25 | SEC-03: sviluppo su loopback per impostazione predefinita | Riduce l'esposizione accidentale di un server senza hardening; l'accesso LAN richiede ora un comando esplicito e documentato |
| 2026-09-26 | SEC-04: CI read-only e action fissate a commit verificati | Limita il token del workflow, riduce il rischio di supply-chain sulle action e automatizza gli aggiornamenti con Dependabot |
| 2026-09-26 | SEC-05: CSP enforced con Trusted Types in osservazione | La policy applicata blocca origini inattese; l'unica eccezione remota è l'immagine Unsplash esistente e Trusted Types resta misurabile senza rischiare regressioni |
| 2026-09-26 | SEC-06: gate High/Critical, scanner complementari e soppressioni versionate | Mantiene bloccanti gli advisory più gravi e i segreti, rende visibili anche i finding informativi via SARIF e impedisce eccezioni anonime o senza scadenza |
| 2026-09-26 | SEC-07: validatori puri condivisi senza nuova dipendenza | Le primitive esistenti erano corrette ma gli input le invocavano con coercizioni diverse; un solo livello conserva gli stati di editing, uniforma limiti/errori IT/EN e rende testabili i casi ostili |
| 2026-09-26 | SEC-08: schema locale allowlist, versione `1` e migrazione conservativa | Le quattro preferenze non sono segreti e non richiedono cifratura; validazione per campo, merge sicuro e fallback dei formati sconosciuti impediscono a dati corrotti o manomessi di entrare nello stato applicativo |
| 2026-09-26 | SEC-09: classificazione semantica e redazione alla sorgente | Normalizzare i nomi delle chiavi copre varianti realistiche senza una regex permissiva; eliminare il valore già dal risultato dell'ispezione evita che futuri consumer possano mostrarlo accidentalmente |
...
| 2026-09-24 | ENG-02: audit runtime bloccante, toolchain inizialmente solo report | La misura transitoria ha mantenuto la CI utile durante SEC-01; SEC-06 ha poi promosso l'audit completo a gate High/Critical dopo l'azzeramento degli advisory |
