...
/**
 * One line of captured output. The id is what annotations bind to: binding by position
 * meant that inserting or reordering a line silently moved every annotation after it
 * onto the wrong text, with nothing failing.
 */
...
export interface EvidenceAnnotation {
  /** Id of the output line this annotation explains. */
  lineId: string;
...
const a = (lineId: string, severity: EvidenceSeverity, label: Bilingual, meaning: Bilingual): EvidenceAnnotation => ({ lineId, severity, label, meaning });
/** Declares an output line: o('crc', '  18422 input errors, ...'). */
const o = (id: string, text: string): EvidenceLine => ({ id, text });
...
export const SECURITY_EVIDENCE_CASES: SecurityEvidenceCase[] = [
  {
    ...
    output: [o('state', 'GigabitEthernet0/1 is up, line protocol is up'), o('bw', '  MTU 1500 bytes, BW 100000 Kbit/sec, DLY 100 usec'), o('duplex', '  Half-duplex, 100Mb/s, media type is RJ45'), o('rate', '  5 minute input rate 41000000 bits/sec, 5100 packets/sec'), o('errors', '  18422 input errors, 17301 CRC, 0 frame, 0 overrun'), o('collisions', '  921 late collisions, 0 deferred, 0 lost carrier')],
    annotations: [a('duplex', 'warning', b('Velocità e duplex negoziati', 'Negotiated speed and duplex'), b('Una porta Gigabit finita a 100 Mb/s half-duplex è già un segnale: l’autonegoziazione non è riuscita e il capo opposto è probabilmente forzato.', 'A Gigabit port that ended up at 100 Mb/s half duplex is already a signal: autonegotiation failed and the far end is probably hard-coded.')), a('errors', 'critical', b('CRC elevati', 'High CRC count'), b('Indica corruzione Layer 1: cablaggio, transceiver, interferenza o porta.', 'Indicates Layer 1 corruption: cabling, transceiver, interference, or port.')), a('collisions', 'critical', b('Late collision', 'Late collision'), b('Le late collision esistono solo in half-duplex, dove la rilevazione delle collisioni è attiva: sono la firma del lato half di un duplex mismatch. Sul capo opposto, in full-duplex, non vedrai collisioni ma CRC e input error che crescono con il carico.', 'Late collisions exist only in half duplex, where collision detection is active: they are the signature of the half-duplex side of a duplex mismatch. On the far end, in full duplex, you will see no collisions at all but CRC and input errors that grow with load.'))],
  ...
  },
  {
    ...
    output: [o('header', 'Vlan      Forwarded        Dropped     DHCP Drops   ACL Drops'), o('rule', '----      ---------        -------     ----------   ---------'), o('vlan20', '  20          182440            317            317           0'), o('vlan30', '  30           92201              0              0           0'), o('interface', 'Interface Gi1/0/18: rate 14 pps, 301 validation failures'), o('syslog', '%SW_DAI-4-DHCP_SNOOPING_DENY: 2 Invalid ARPs on Gi1/0/18')],
    annotations: [a('vlan20', 'warning', b('Drop concentrati', 'Concentrated drops'), b('La VLAN 20 mostra ARP non coerenti con i binding DHCP Snooping.', 'VLAN 20 shows ARP inconsistent with DHCP Snooping bindings.')), a('interface', 'critical', b('Porta sorgente', 'Source port'), b('Riga che arriva da `show ip arp inspection interfaces`, non dalle statistiche per VLAN: Gi1/0/18 concentra failure e consente di ridurre il perimetro.', 'This line comes from `show ip arp inspection interfaces`, not from the per-VLAN statistics: Gi1/0/18 concentrates failures and narrows the scope.')), a('syslog', 'context', b('Evento di enforcement', 'Enforcement event'), b('Questa riga è un messaggio syslog, non output di show. DAI ha bloccato frame: è evidenza di controllo, non identità certa dell’attaccante.', 'This line is a syslog message, not show output. DAI blocked frames: this proves enforcement, not the attacker’s identity.'))],
  ...
  },
  {
    ...
    output: [o('neighbor', 'Neighbor 10.0.0.2, interface address 10.12.0.2'), o('area', '  In the area 0 via interface GigabitEthernet0/0'), o('state', '  Neighbor priority is 1, State is FULL, 6 state changes'), o('dead', '  Dead timer due in 00:00:03'), o('uptime', '  Neighbor is up for 00:04:12'), o('retransmit', '  Index 1/1, retransmission queue length 28, number of retransmission 146')],
    annotations: [a('state', 'warning', b('State change', 'State changes'), b('Sei transizioni in pochi minuti indicano instabilità, non necessariamente attacco.', 'Six transitions in a few minutes indicate instability, not necessarily an attack.')), a('dead', 'critical', b('Dead timer vicino a zero', 'Dead timer near zero'), b('Hello mancanti possono far cadere l’adiacenza alla prossima scadenza.', 'Missing Hellos may drop the adjacency at the next expiry.')), a('retransmit', 'warning', b('Ritrasmissioni', 'Retransmissions'), b('Queue e counter elevati suggeriscono loss, MTU o congestione del control plane.', 'High queue and counters suggest loss, MTU issues, or control-plane congestion.'))],
  ...
  },
  {
    ...
    output: [o('baseline', 'Baseline: 203.0.113.0/24  origin AS64520  RPKI Valid'), o('observed', 'Observed: 203.0.113.0/25 origin AS64599  RPKI Invalid'), o('aspath', 'AS_PATH: 64496 64599'), o('firstseen', 'First seen: 14:32:08 UTC; collectors: 19/24'), o('shift', 'Traffic shift: 72% toward new origin')],
    annotations: [a('observed', 'critical', b('Più specifica e invalid', 'More specific and invalid'), b('La /25 vince il longest-prefix match e l’origine non è autorizzata dal ROA.', 'The /25 wins longest-prefix match and its origin is not authorized by the ROA.')), a('firstseen', 'context', b('Visibilità multipla', 'Multiple vantage points'), b('Più collector riducono la probabilità di un’anomalia locale.', 'Multiple collectors reduce the likelihood of a local observation artifact.')), a('shift', 'warning', b('Impatto osservabile', 'Observable impact'), b('Il cambio di traffico sostiene l’ipotesi di dirottamento.', 'The traffic shift supports the diversion hypothesis.'))],
  ...
  },
  {
    ...
    output: [o('total', 'Total active translations: 65512 (4096 static, 61416 dynamic; 61416 extended)'), o('peak', 'Peak translations: 65535, occurred 00:01:42 ago'), o('outside', 'Outside interfaces: GigabitEthernet0/0'), o('inside', 'Inside interfaces: Vlan10, Vlan20, Vlan30'), o('hits', 'Hits: 99824103  Misses: 128744'), o('failures', 'Expired translations: 781  Allocation failures: 120493')],
    annotations: [a('total', 'warning', b('Tabella quasi piena', 'Nearly full table'), b('Il numero è vicino al picco dichiarato e indica pressione sullo stato.', 'The number is near the reported peak and indicates state pressure.')), a('peak', 'critical', b('Picco massimo', 'Maximum peak'), b('La capacità è stata saturata recentemente.', 'Capacity was saturated recently.')), a('failures', 'critical', b('Allocation failure', 'Allocation failures'), b('Le nuove traduzioni non possono essere create: spiega il sintomo osservato.', 'New translations cannot be created: this explains the observed symptom.'))],
  ...
  },
  {
    ...
    output: [o('auth', '22:14:02 AUTH SUCCESS user=netops src=198.51.100.24 method=ssh'), o('author', '22:14:04 AUTHOR cmd="configure terminal" privilege=15 permit'), o('acct-logging', '22:14:11 ACCT cmd="no logging host 10.10.10.50" result=success'), o('acct-user', '22:14:18 ACCT cmd="username backup privilege 15 secret *****" result=success'), o('syslog-down', '22:15:00 SYSLOG transport to 10.10.10.50 unavailable'), o('auth-backup', '22:18:42 AUTH SUCCESS user=backup src=198.51.100.24 method=ssh')],
    annotations: [a('auth', 'warning', b('Sorgente esterna', 'External source'), b('L’accesso privilegiato proviene da un indirizzo da validare rispetto alla management policy.', 'Privileged access comes from an address that must be checked against management policy.')), a('acct-logging', 'critical', b('Riduzione visibilità', 'Reduced visibility'), b('La rimozione del collector precede altre modifiche e può indicare defense evasion.', 'Removing the collector precedes other changes and may indicate defense evasion.')), a('acct-user', 'critical', b('Nuova persistenza', 'New persistence'), b('Un account privilegiato viene creato e poi utilizzato dalla stessa sorgente.', 'A privileged account is created and then used from the same source.'))],
  ...
  },
  {
    ...
    output: [o('session', 'Session-id: 42, Status: UP-ACTIVE, IKE count: 1, CHILD count: 1'), o('remoteid', 'Remote id: vpn-branch.example.net'), o('certificate', 'Certificate serial: 07:A9:41:CC, issuer: CN=Example-Issuing-CA-2'), o('validation', 'Chain validation: SUCCESS; revocation status: UNKNOWN'), o('suite', 'Encryption: AES-CBC-128, Integrity: SHA1, PRF: SHA1, DH group: 2'), o('lifetime', 'Life/Active Time: 86400/3812 sec')],
    annotations: [a('certificate', 'context', b('Nuovo certificato', 'New certificate'), b('Seriale e issuer devono essere confrontati con inventario e change record.', 'Serial and issuer must be compared with inventory and change records.')), a('validation', 'warning', b('Revoca sconosciuta', 'Unknown revocation'), b('Chain valida non significa che il certificato non sia stato revocato.', 'A valid chain does not mean the certificate has not been revoked.')), a('suite', 'critical', b('Suite legacy', 'Legacy suite'), b('SHA-1 e DH group 2 (1024 bit) sono parametri deboli da sostituire secondo compatibilità e policy. La PFS del rekey dei CHILD_SA non compare in questo output: si configura nell’IPsec profile e va verificata a parte.', 'SHA-1 and DH group 2 (1024-bit) are weak parameters to replace according to compatibility and policy. PFS for CHILD_SA rekeying does not appear in this output: it is set in the IPsec profile and must be verified separately.'))],
  ...
  },
  {
    ...
    output: [o('run', 'run_id=8421 actor=svc-netauto artifact=net-policy@4f9c2a1 signature=VALID'), o('requested', 'scope requested=site-rome devices=12'), o('resolved', 'scope resolved=region-emea devices=418'), o('plan', 'plan: +0 ~836 -0 resources; approval=auto'), o('apply', 'apply status=SUCCESS changed_devices=417 failed_devices=1'), o('postcheck', 'post-check drift=173 devices; rollback_test=NOT_RUN')],
    annotations: [a('run', 'context', b('Provenienza valida', 'Valid provenance'), b('La firma valida identifica l’artifact, non garantisce che la logica sia corretta.', 'A valid signature identifies the artifact; it does not guarantee correct logic.')), a('resolved', 'critical', b('Scope expansion', 'Scope expansion'), b('Il target risolto è molto più ampio della richiesta: definisce il blast radius.', 'The resolved target is much broader than requested and defines the blast radius.')), a('plan', 'warning', b('Approvazione automatica', 'Automatic approval'), b('836 modifiche senza gate umano aumentano il rischio sistemico.', '836 changes without a human gate increase systemic risk.')), a('postcheck', 'critical', b('Drift post-change', 'Post-change drift'), b('Success della pipeline non coincide con convergenza reale e il rollback non è stato testato.', 'Pipeline success does not equal actual convergence, and rollback was not tested.'))],
