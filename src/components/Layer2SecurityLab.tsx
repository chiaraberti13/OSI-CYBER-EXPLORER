...
import { AlertTriangle, Cable, CheckCircle2, Filter, Network, Search, ShieldCheck } from 'lucide-react';
...
const PLANES: SecurityPlane[] = ['physical', 'data', 'control', 'management', 'identity'];
const AREA_LABELS: Record<Layer2SecurityArea, { it: string; en: string }> = {
  physical: { it: 'Confine fisico', en: 'Physical boundary' }, 'edge-port': { it: 'Porta edge', en: 'Edge port' },
  cam: { it: 'CAM e Port Security', en: 'CAM and Port Security' }, trunk: { it: 'Trunk e VLAN', en: 'Trunks and VLANs' },
  dhcp: { it: 'DHCP Snooping', en: 'DHCP Snooping' }, arp: { it: 'DAI e Source Guard', en: 'DAI and Source Guard' },
  stp: { it: 'STP e loop', en: 'STP and loops' }, etherchannel: { it: 'EtherChannel', en: 'EtherChannel' },
  containment: { it: 'Contenimento', en: 'Containment' }
};
...
export default function Layer2SecurityLab() {
  const language = useStore(state => state.language);
  const [area, setArea] = useState<'all' | Layer2SecurityArea>('all');
  const [plane, setPlane] = useState<'all' | SecurityPlane>('all');
  const filtered = useMemo(() => LAYER2_SECURITY_SCENARIOS.filter(item => (area === 'all' || item.area === area) && (plane === 'all' || item.planes.includes(plane))), [area, plane]);

  return (
    <div className="space-y-6">
      <header className="rounded-xl border border-slate-200 bg-white p-6 md:p-8"><div className="flex items-start gap-4"><div className="rounded-xl bg-emerald-50 p-3 text-emerald-800"><Cable className="h-6 w-6" aria-hidden="true" /></div><div><p className="eyebrow">PORT · FRAME · BINDING · TOPOLOGY · PROOF</p><h1 className="mt-2 text-2xl font-semibold text-slate-900">{language === 'it' ? 'Laboratorio sicurezza Layer 2 e first-hop' : 'Layer 2 & First-Hop Security Lab'}</h1><p className="mt-2 max-w-4xl text-sm leading-relaxed text-slate-600">{language === 'it' ? 'Segui il frame dal mezzo fisico fino a VLAN, CAM, binding, ARP e topologia. Gli scenari collegano ruolo della porta, trust e forwarding effettivo senza fermarsi allo stato “up”.' : 'Follow a frame from the physical medium through VLANs, CAM, bindings, ARP, and topology. Scenarios connect port roles, trust, and actual forwarding without stopping at an “up” state.'}</p></div></div></header>
      <section className="grid gap-3 sm:grid-cols-3" aria-label={language === 'it' ? 'Riepilogo sicurezza Layer 2' : 'Layer 2 security summary'}>{[{ value: LAYER2_SECURITY_SCENARIOS.length, it: 'Scenari operativi', en: 'Operational scenarios' }, { value: new Set(LAYER2_SECURITY_SCENARIOS.flatMap(item => item.techniqueIds)).size, it: 'Tecniche correlate', en: 'Related techniques' }, { value: LAYER2_SECURITY_SCENARIOS.reduce((total, item) => total + item.verification.length, 0), it: 'Verifiche operative', en: 'Operational checks' }].map(item => <div key={item.en} className="rounded-xl border border-slate-200 bg-white p-4"><p className="text-2xl font-semibold text-slate-900">{item.value}</p><p className="text-xs text-slate-500">{item[language]}</p></div>)}</section>
      <aside className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-4 text-emerald-950"><p className="flex items-center gap-2 text-sm font-semibold"><ShieldCheck className="h-4 w-4" aria-hidden="true" />{language === 'it' ? 'Principio first-hop' : 'First-hop principle'}</p><p className="mt-2 text-xs leading-relaxed">{language === 'it' ? 'Trust e protezioni dipendono dal ruolo della porta. Una configurazione sicura su un edge può causare outage su un uplink; verificare sempre peer, VLAN, binding, topologia e flusso reale.' : 'Trust and protections depend on port role. A secure edge configuration may cause an outage on an uplink; always verify peers, VLANs, bindings, topology, and actual flows.'}</p></aside>
      <section className="rounded-xl border border-slate-200 bg-white p-4" aria-labelledby="layer2-filter-title"><h2 id="layer2-filter-title" className="flex items-center gap-2 text-sm font-semibold text-slate-900"><Filter className="h-4 w-4 text-emerald-800" aria-hidden="true" />{language === 'it' ? 'Filtra gli scenari' : 'Filter scenarios'}</h2><div className="mt-3 grid gap-3 sm:grid-cols-2"><FilterSelect label={language === 'it' ? 'Area Layer 2' : 'Layer 2 area'} value={area} onChange={value => setArea(value as 'all' | Layer2SecurityArea)} allLabel={language === 'it' ? 'Tutte le aree' : 'All areas'} options={AREAS.map(value => ({ value, label: AREA_LABELS[value][language] }))} /><FilterSelect label={language === 'it' ? 'Piano di sicurezza' : 'Security plane'} value={plane} onChange={value => setPlane(value as 'all' | SecurityPlane)} allLabel={language === 'it' ? 'Tutti i piani' : 'All planes'} options={PLANES.map(value => ({ value, label: PLANE_LABELS[value][language] }))} /></div></section>
      <div className="flex items-center justify-between gap-4"><h2 className="text-lg font-semibold text-slate-900">{language === 'it' ? 'Percorso del frame, minaccia e verifica' : 'Frame path, threat, and verification'}</h2><span className="text-xs font-semibold text-slate-500" aria-live="polite">{filtered.length} {language === 'it' ? 'risultati' : 'results'}</span></div>
      {filtered.length > 0 ? <section className="grid gap-4 xl:grid-cols-2" aria-label={language === 'it' ? 'Scenari Layer 2 e first-hop' : 'Layer 2 and first-hop scenarios'}>{filtered.map(item => <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-5 [content-visibility:auto] [contain-intrinsic-size:auto_780px]">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 pb-4"><div><p className="text-[10px] font-semibold uppercase tracking-wider text-emerald-800">{AREA_LABELS[item.area][language]}</p><h3 className="mt-1 text-base font-semibold text-slate-900">{item.title[language]}</h3></div><div className="flex flex-wrap justify-end gap-1">{item.planes.map(value => <span key={value} className="rounded-full bg-indigo-50 px-2 py-1 text-[9px] font-semibold text-indigo-800">{PLANE_LABELS[value][language]}</span>)}</div></div>
        <Info icon={Network} label={language === 'it' ? 'Percorso del frame' : 'Frame path'} tone="sky"><ol className="space-y-2">{item.framePath.map((step, index) => <li key={step.en} className="flex gap-2"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-sky-200 text-[9px] font-bold text-sky-900">{index + 1}</span><span>{step[language]}</span></li>)}</ol></Info>
