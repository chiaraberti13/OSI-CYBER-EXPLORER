...
import { AlertTriangle, CheckCircle2, Filter, Network, RadioTower, Search, ShieldCheck } from 'lucide-react';
...
const AREA_LABELS: Record<ManagementTelemetryArea, { it: string; en: string }> = {
  boundary: { it: 'Confine di gestione', en: 'Management boundary' }, ssh: { it: 'Accesso SSH', en: 'SSH access' },
  snmp: { it: 'SNMP', en: 'SNMP' }, syslog: { it: 'Syslog', en: 'Syslog' }, time: { it: 'Tempo e NTP', en: 'Time and NTP' },
  api: { it: 'API di gestione', en: 'Management APIs' }, discovery: { it: 'Discovery', en: 'Discovery' },
  backup: { it: 'Backup e restore', en: 'Backup and restore' }, collectors: { it: 'Collector e copertura', en: 'Collectors and coverage' }
};
...
export default function ManagementTelemetryLab() {
  const language = useStore(state => state.language);
  const [area, setArea] = useState<'all' | ManagementTelemetryArea>('all');
  const [plane, setPlane] = useState<'all' | SecurityPlane>('all');
  const filtered = useMemo(() => MANAGEMENT_TELEMETRY_SCENARIOS.filter(item => (area === 'all' || item.area === area) && (plane === 'all' || item.planes.includes(plane))), [area, plane]);

  return (
    <div className="space-y-6">
      <header className="rounded-xl border border-slate-200 bg-white p-6 md:p-8"><div className="flex items-start gap-4"><div className="rounded-xl bg-cyan-50 p-3 text-cyan-800"><RadioTower className="h-6 w-6" aria-hidden="true" /></div><div><p className="eyebrow">REACHABILITY · TRUST · TELEMETRY · PROOF</p><h1 className="mt-2 text-2xl font-semibold text-slate-900">{language === 'it' ? 'Laboratorio management plane e telemetria' : 'Management Plane & Telemetry Lab'}</h1><p className="mt-2 max-w-4xl text-sm leading-relaxed text-slate-600">{language === 'it' ? 'Segui la catena da accesso amministrativo, identità e protocolli sicuri fino a collector, tempo, backup e prova operativa. Ogni scenario evidenzia ciò che un singolo stato “up” o evento ricevuto non può dimostrare.' : 'Follow the chain from administrative access, identity, and secure protocols to collectors, time, backups, and operational proof. Each scenario highlights what a single “up” state or received event cannot establish.'}</p></div></div></header>

      <section className="grid gap-3 sm:grid-cols-3" aria-label={language === 'it' ? 'Riepilogo management e telemetria' : 'Management and telemetry summary'}>{[{ value: MANAGEMENT_TELEMETRY_SCENARIOS.length, it: 'Scenari operativi', en: 'Operational scenarios' }, { value: new Set(MANAGEMENT_TELEMETRY_SCENARIOS.flatMap(item => item.techniqueIds)).size, it: 'Tecniche correlate', en: 'Related techniques' }, { value: MANAGEMENT_TELEMETRY_SCENARIOS.reduce((total, item) => total + item.verification.length, 0), it: 'Verifiche end-to-end', en: 'End-to-end checks' }].map(item => <div key={item.en} className="rounded-xl border border-slate-200 bg-white p-4"><p className="text-2xl font-semibold text-slate-900">{item.value}</p><p className="text-xs text-slate-500">{item[language]}</p></div>)}</section>

      <aside className="rounded-xl border border-cyan-200 bg-cyan-50/70 p-4 text-cyan-950"><p className="flex items-center gap-2 text-sm font-semibold"><ShieldCheck className="h-4 w-4" aria-hidden="true" />{language === 'it' ? 'Principio di assurance' : 'Assurance principle'}</p><p className="mt-2 text-xs leading-relaxed">{language === 'it' ? 'Raggiungibilità, cifratura, autenticazione e ricezione di un evento sono prove diverse. Correlare sempre percorso, identità e autorizzazione, stato del protocollo, salute della pipeline e una fonte indipendente.' : 'Reachability, encryption, authentication, and event receipt are different proofs. Always correlate path, identity and authorization, protocol state, pipeline health, and an independent source.'}</p></aside>

      <section className="rounded-xl border border-slate-200 bg-white p-4" aria-labelledby="management-filter-title"><h2 id="management-filter-title" className="flex items-center gap-2 text-sm font-semibold text-slate-900"><Filter className="h-4 w-4 text-cyan-800" aria-hidden="true" />{language === 'it' ? 'Filtra gli scenari' : 'Filter scenarios'}</h2><div className="mt-3 grid gap-3 sm:grid-cols-2"><FilterSelect label={language === 'it' ? 'Area operativa' : 'Operational area'} value={area} onChange={value => setArea(value as 'all' | ManagementTelemetryArea)} allLabel={language === 'it' ? 'Tutte le aree' : 'All areas'} options={AREAS.map(value => ({ value, label: AREA_LABELS[value][language] }))} /><FilterSelect label={language === 'it' ? 'Piano di sicurezza' : 'Security plane'} value={plane} onChange={value => setPlane(value as 'all' | SecurityPlane)} allLabel={language === 'it' ? 'Tutti i piani' : 'All planes'} options={PLANES.map(value => ({ value, label: PLANE_LABELS[value][language] }))} /></div></section>

      <div className="flex items-center justify-between gap-4"><h2 className="text-lg font-semibold text-slate-900">{language === 'it' ? 'Catena di fiducia, minaccia e verifica' : 'Trust chain, threat, and verification'}</h2><span className="text-xs font-semibold text-slate-500" aria-live="polite">{filtered.length} {language === 'it' ? 'risultati' : 'results'}</span></div>
      {filtered.length > 0 ? <section className="grid gap-4 xl:grid-cols-2" aria-label={language === 'it' ? 'Scenari management e telemetria' : 'Management and telemetry scenarios'}>{filtered.map(item => <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-5 deferred-card [contain-intrinsic-height:auto_780px]">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 pb-4"><div><p className="text-[10px] font-semibold uppercase tracking-wider text-cyan-800">{AREA_LABELS[item.area][language]}</p><h3 className="mt-1 text-base font-semibold text-slate-900">{item.title[language]}</h3></div><div className="flex flex-wrap justify-end gap-1">{item.planes.map(value => <span key={value} className="rounded-full bg-indigo-50 px-2 py-1 text-[9px] font-semibold text-indigo-800">{PLANE_LABELS[value][language]}</span>)}</div></div>
        <Info icon={Network} label={language === 'it' ? 'Catena di fiducia' : 'Trust path'} tone="sky"><ol className="space-y-2">{item.trustPath.map((step, index) => <li key={step.en} className="flex gap-2"><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-sky-200 text-[9px] font-bold text-sky-900">{index + 1}</span><span>{step[language]}</span></li>)}</ol></Info>
