...
export default function AutomationLab() {
  ...
  const jsonInspection = useMemo(() => {
    try {
      ...
      return { nodes: [], sensitive: [], error } as const;
    }
  }, [jsonInput]);
  const validatedResource = useMemo(() => {
    try {
      return { value: parseRelativeResourcePath(resource), error: null } as const;
    } catch (error) {
      return { value: null, error } as const;
    }
  }, [resource]);
  const validatedStatus = useMemo(() => {
    try {
      return { value: parseHttpStatus(status), error: null } as const;
    } catch (error) {
      return { value: null, error } as const;
    }
  }, [status]);
  const profile = httpMethodProfile(method);
  const statusFamily = validatedStatus.value === null ? null : httpStatusFamily(validatedStatus.value);
  ...
  const t = language === 'it'
    ? {
        ...
        rest: 'REST request explorer', method: 'Metodo HTTP', resource: 'Resource URI', status: 'Status simulato', crud: 'Operazione CRUD', safe: 'Safe', idempotent: 'Idempotente', success: 'Successi tipici', yes: 'Sì', no: 'No', restNote: 'Safe significa che il metodo non dovrebbe modificare lo stato. Idempotente significa che ripetere la stessa richiesta produce lo stesso stato finale; non garantisce una risposta identica.', simulationOnly: 'Simulazione locale: nessuna richiesta di rete viene inviata.',
    ...
    : {
        ...
        rest: 'REST request explorer', method: 'HTTP method', resource: 'Resource URI', status: 'Simulated status', crud: 'CRUD operation', safe: 'Safe', idempotent: 'Idempotent', success: 'Typical success', yes: 'Yes', no: 'No', restNote: 'Safe means the method should not modify state. Idempotent means repeating the same request produces the same final state; it does not guarantee an identical response.', simulationOnly: 'Local simulation: no network request is sent.',
