...
export default function WirelessSecurityLab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {filtered.length > 0 ? <section className="grid gap-4 xl:grid-cols-2" aria-label={language === 'it' ? 'Scenari sicurezza wireless' : 'Wireless security scenarios'}>{filtered.map(item => <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-5 deferred-card [contain-intrinsic-height:auto_720px]">
