...
export default function AvailabilitySecurityLab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {filtered.length > 0 ? <section className="grid gap-4 xl:grid-cols-2" aria-label={language === 'it' ? 'Scenari disponibilità e DoS' : 'Availability and DoS scenarios'}>{filtered.map(item => <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-5 [content-visibility:auto] [contain-intrinsic-size:auto_760px]">
